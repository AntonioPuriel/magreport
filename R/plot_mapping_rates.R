...
#' in the assembly, which is worth checking before interpreting coverage.
#'
...
plot_mapping_rates <- function(mapping, warn_below = 70) {
  ...
  mapping$sample <- factor(mapping$sample, levels = mapping$sample[order(mapping$overall_alignment_rate_pct)])
